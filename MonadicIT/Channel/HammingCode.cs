...
namespace MonadicIT.Channel
{
    public class HammingCode : IChannelCoder<Binary>
    {
        ...
        {
            get { return ParityBits/(double) BlockLength; }
        }
        ...
        public IEnumerable<Binary> Encode(IEnumerable<Binary> bits)
        {
            ...
            foreach (var block in bits.InChunksOf(ParityBits))
            {
                ...
                int m = 0; // running number of parity bits
                int n = 0; // running number of all bits
                foreach (Binary bit in paddedBlock)
                {
                ...
                foreach (Binary bit in codeBlock)
                {
                    yield return bit;
                }
            }
        ...
        {
            double pe = channel.ErrorRate(); // bit error probability of channel
            ...
            return 1 - MathHelper.KOutOfNProbability(BlockLength, 0, pe) -
                   MathHelper.KOutOfNProbability(BlockLength, 1, pe);
        }
